...
const getPoolAccounts = async (poolAccountData) =>
  await new HttpRequest("api").get(`pool?token=${Cookies.get("token")}`);
...
const validateRejectTransaction = async (payload) => {
  const response = await new HttpRequest("api").post(
...
const SuccessWithdrawPage = () => {
  ...
  useQuery(["getPoolAccounts", poolAccountData], {
    queryFn: async (data) => {
  ...
  const { refetch: refetchAvailableBankAccounts } = useQuery(
    ["getAvaliableBankAccounts", poolAccountData],
    {
      queryFn: async (data) => {
  ...
  const handleBankAccounts = (userid, transactions_id) => {
    setUserId(userid);
    setTransactionId(transactions_id);
    getPoolAccounts(Cookies.get("token"));
    refetchAvailableBankAccounts();
  ...
  const handleValidateRejectTransaction = (userId) => {
  ...
  return (
    <>
      ...
      <Modal
        ...
        className={"w-11/12 mx-4 md:mx-0 md:w-3/4 lg:w-2/4"}
        handleModal={handleBankAccounts}
      >
        <Modal.Header handleModal={handleBankAccounts}>
        ...
        <Modal.Body>
          ...
          <div className="flex gap-2 mt-5">
            <Button
              ...
              onClick={() => {
                handleValidateRejectTransaction(selectedTransactionData._id);
                handleBankAccounts();
              }}
            >
              Reddet
