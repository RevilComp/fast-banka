...
const SuccessDepositPage = () => {
  ...
  const { isLoading: isTransactionDataLoading, refetch: refetchTransaction } =
    useQuery(["getTransactions", transactionParams], {
      ...
      queryFn: async (data) => {
        ...
        if (!response?.length) setHasNext(false);
      },
    });
  ...
  const { refetch: refetchTransactionSummary } = useQuery(
    ["getTransactionsSummary", transactionSummaryParams],
    {
      refetchOnWindowFocus: false,
      queryFn: async (data) => {
          ...
          {(resolvedData) => {
            const tableRows = (search ? searchData : transactionData).map(
              (data) => ({
                ...
                //       variant={"primary"}
                //       onClick={() => {
                //         handleBankAccounts(data.userId, data._id);
                //       }}
                //     >
                //       Düzenle
                //     </Button>
