...
const config = {
    ...
    entegrations:{
        scashmoney:{
            scashmoneySecretKey: process.env.ENTEGRATION_S_CASH_MONEY_SECRET_KEY || '',
            sCashMoneyEnabled: process.env.ENTEGRATION_S_CASH_MONEY == "true" || false,
            sCashMoneyBankApiKey: process.env.ENTEGRATION_S_CASH_MONEY_BANK_API_KEY || '',
            sCashMoneyPaparaApiKey: process.env.ENTEGRATION_S_CASH_MONEY_PAPARA_API_KEY || '',
        }
    }
}
    ...
    for (const key in obj) {
        if (typeof obj[key] === 'object') {
            ...
            if ((obj[key] == undefined || obj[key] == '') && obj[key] != false) {
                console.log("Config file is not configured properly", obj, "key", key,"valu", obj[key])
